...
public class UserNameRecognizer {
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 2:
				if (Character.isLetterOrDigit(currentChar)) {
					...
					userNameRecognizerErrorMessage = "A UserName character must follow '-', '_', or '.'";
	    ...
	    case 0:
	        // State 0 is not a final state, so we return a specific error message
	        userNameRecognizerErrorMessage += "A UserName must start with A-Z or a-z.\n";
	        return userNameRecognizerErrorMessage;

	    case 1:
	        // State 1 is a final state. Check UserName length and ensure full string consumption.
	        if (userNameSize < 4) {
	            ...
	            if (currentChar == '.') {
	                ...
	                moveToNextCharacter(); // Update currentChar and currentCharNdx

	                if (currentCharNdx >= input.length()) {
	                    userNameRecognizerErrorMessage += 
	                        "A UserName cannot end with a period.\n";
	                    return userNameRecognizerErrorMessage;
	                }

	                // Check if the nextChar is valid (A-Z, a-z, 0-9)
	                if (!Character.isLetterOrDigit(currentChar)) {
	                    userNameRecognizerErrorMessage += 
	                        "A UserName character after a period must be A-Z, a-z, or 0-9.\n";
	                    return userNameRecognizerErrorMessage;
	                }

	                nextState = 2;  // Transition back to state 2 after period and valid next character
	                userNameSize++;
	            } else if (currentChar == '_') {
	                // Transition to a new state to check for trailing underscores
	                nextState = 3; 
	                moveToNextCharacter();
	            } else {
	                userNameRecognizerErrorMessage += 
	                    "A UserName may only contain A-Z, a-z, 0-9, '.', or '_'.\n";
	                return userNameRecognizerErrorMessage;
	            }
	        } else {
	            // UserName is valid
	    ...
	    case 2:
	        ...
	        if (currentCharNdx >= input.length()) {
	            userNameRecognizerErrorMessage += "A UserName cannot end with an underscore.\n";
	            return userNameRecognizerErrorMessage;
	        } else if (!Character.isLetterOrDigit(currentChar) && currentChar != '.') {
	            userNameRecognizerErrorMessage += 
	                "An underscore must be followed by A-Z, a-z, 0-9.\n";
	            return userNameRecognizerErrorMessage;
	        }
	        nextState = 1;
	        break;
	    ...
	    default:
	        ...
	        userNameRecognizerErrorMessage += "A UserName cannot end with an underscore, period, or minus.\n";
	        return userNameRecognizerErrorMessage;
	}
		return input;
